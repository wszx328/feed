...
function initGameState(carriedItemIds) {
  ...
  gameState = {
    ...
    itemUses: {},
    gloveMultiplier: 1,
    pendingDiceFoodId: null,     // 占卜骰子指定下一轮食物（旧逻辑保留字段，不再使用）
    activeDiceFoodId: null,      // 当前轮已生效的骰子指定食物
    diceCorrectCount: 0,
    diceDefied: false,
...
function useDivinationDice(foodId) {
  ...
  // 直接设置 activeDiceFoodId，下一轮生成时使用（这里“下一轮”指即将刷新的当前轮）
  state.activeDiceFoodId = foodId;
  state.pendingDiceFoodId = null; // 不再使用延迟字段
